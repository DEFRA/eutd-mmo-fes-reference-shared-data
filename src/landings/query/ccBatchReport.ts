  ...
  return Transformations.imap(
    ...
    if (q.landingTotalBreakdown) {
      ...
      q.landingTotalBreakdown
        .map(landingAggregatedItemBreakdown => { return `species: ${q.species}, ` +
                `${(landingAggregatedItemBreakdown.presentation !== undefined) ? `presentation: ${landingAggregatedItemBreakdown.presentation}, `:''}`+
                `${(landingAggregatedItemBreakdown.state !== undefined) ? `state: ${landingAggregatedItemBreakdown.state}, `:''}` +
                ...
                `${(landingAggregatedItemBreakdown.isEstimate) ? `estimate weight plus tolerance: ${landingAggregatedItemBreakdown.liveWeight + landingAggregatedItemBreakdown.liveWeight * 0.1}, `:''}` +
   ...
   if (moment.duration(unavailabilityDuration) < moment.duration())
    ...
    if (!q.isLandingExists) {
      ...
      if ((moment.duration(q.durationBetweenCertCreationAndFirstLandingRetrieved)) > moment.duration(14, 'days'))
        r.FI0_47_unavailabilityExceeds14Days = 'Fail'
      else
